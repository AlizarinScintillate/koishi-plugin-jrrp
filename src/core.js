	    ...
	    .action(({ session }) => {
	      ...
	      const renderResult = comment => {
	        if (config.result) {
	          ...
	          return session.text('qiuqian.result', [name, luckValue, comment])
	        }
	      }
	      ...
	      if (config.useJackpot) {
	        ...
	        if (jackpotIndex != -1) {
	          if (hasCustomJackpots) {
	            ...
	            comment = session.text(`qiuqian.default-jackpot-${luckValue}`)
	          }
	        }
	      }
	      ...
	      if (!comment) {
	        ...
	        if (hasCustumLevels) {
	          ...
	          comment = session.text(`qiuqian.default-level-${key}`)
	        }
	      }
